...
- **Visual requirements carry through**: If your External Data Requirements identify visual assets (images, thumbnails, maps) that a sidekick task will be responsible for gathering, you MUST explicitly include those requirements in the task instructions. Without explicit instructions (and output schemas that include image fields), sidekick tasks are unlikely to prioritize gathering visual content.
...
8. **Sidekick Task Evaluation**: Look at your workflow as a whole and ask: are there groups of steps that together require understanding, judgment, or synthesis across multiple data sources? If so, those steps are strong candidates to be combined into a sidekick task rather than implemented as direct tool calls and `sdk.callLLM`. This is especially true for background operations where latency is acceptable. Document your decision and reasoning in the data plan.
   - **Warning:** Steps often appear simpler than they are. "Search email for briefing material" sounds like a tool call but requires judgment about what to search for, how to evaluate results, and what to include. If multiple steps each require a `callLLM` to figure out how to proceed, you're building a fragile orchestration chain — use a sidekick task instead. Let the task make related tool calls directly, and use `completionCallback` to persist structured results in your database (the task can't write to your DB). See "Prefer Keeping Tool Calls in the Task" in `skills/sidekick-tasks/SKILL.md` for details.
...
For each requirement, consider both the data AND any visual assets (images, maps, icons) that would enhance the UI:

1. [Requirement 1 description]
   - Visual: [e.g., "Event images from venue photos or OpenGraph metadata"]
2. [Requirement 2 description]
   - Visual: [e.g., "None needed" or "Map preview of location"]
...
...
If rejecting a sidekick task approach, verify your reasoning:
- Can `completionCallback` handle your storage and follow-up needs? (Yes — it calls your exported server function with full DB/tool access)
- Can you prompt cost limits into the task instructions? (Yes — e.g., "check at most 5 emails per attendee")
- Do multiple steps require judgment about how to use tools, interpret results, or adapt based on what's discovered? (If yes → sidekick task)

If using a sidekick task, verify the workflow boundary:
- Are there tool calls in your callback that the task could handle directly?
- Is `completionCallback` focused on database persistence and state coordination, or is it also making tool calls the task could own?
...
**Visual/media requirements**: [What visual assets should this task gather? e.g., "Extract event images and OpenGraph thumbnails from crawled URLs", "Include venue photos", "None". Cross-reference the External Data Requirements — any visual needs handled by this task must be listed here and included in the instructions.]
...
> Consider: For steps that are part of the same workflow as the sidekick task
> (e.g., making tool calls based on the task's analysis), could the task handle
> them directly? Tasks adapt to failures and unexpected data in ways that
> callback code can't easily replicate.
...
## Visual Asset Persistence (if applicable)
If your data plan includes visual assets from external sources, consider whether to persist them:

| Source | Persist? | Reason |
|--------|----------|--------|
| [e.g., Event images from web scraping] | [Yes/No] | [e.g., "URLs may expire or change"] |
| [e.g., OpenGraph thumbnails] | [Yes/No] | [e.g., "URLs are stable, no persistence needed"] |

**Persist when**: URLs are ephemeral, source is unreliable, or you need guaranteed availability.
**Skip when**: URLs are stable and served from reliable CDNs (persistence adds overhead without benefit).
**Skill reference**: See `file-storage` skill for upload patterns.

## Rejected Approaches
If you considered but rejected a sidekick task approach, verify your reasons against the capabilities in `skills/sidekick-tasks/SKILL.md` (especially `completionCallback`, `progressCallback`, and prompted cost control) before finalizing.
