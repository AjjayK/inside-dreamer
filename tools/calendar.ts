...
export async function listAccounts(
  ...
  return await sdk.callTool("calendar/1.2.0/listAccounts", params) as listAccountsOutput;
}
...
export async function listCalendars(
  ...
  return await sdk.callTool("calendar/1.2.0/listCalendars", params) as listCalendarsOutput;
}
...
export async function createCalendar(
  ...
  return await sdk.callTool("calendar/1.2.0/createCalendar", params) as createCalendarOutput;
}
...
export async function updateCalendar(
  ...
  return await sdk.callTool("calendar/1.2.0/updateCalendar", params) as updateCalendarOutput;
}
...
export async function getEventsForDay(
  ...
  return await sdk.callTool("calendar/1.2.0/getEventsForDay", params) as getEventsForDayOutput;
}
...
export type getUpcomingEventsParams = {
  // Email address of the calendar account from listAccounts
  account: string,
  // RFC3339 formatted timestamp. Returns events that start BEFORE this time (i.e., excludes events starting at or after this point). Together with startDate, these filters return all events overlapping the time window. MUST include timezone offset matching the user's local timezone (e.g., '2024-01-20T00:00:00-05:00').
  endDate?: string,
  // Pagination token from previous response's nextPageToken field. Only provide this if you need MORE results beyond what maxResults returned. If maxResults gave you enough events, do NOT fetch additional pages.
  pageToken?: (string | null),
  // RFC3339 formatted timestamp. Returns events that end AFTER this time (i.e., excludes events already finished before this point). MUST include timezone offset matching the user's local timezone, not UTC (e.g., '2024-01-15T00:00:00-05:00' for midnight Eastern Time, NOT '2024-01-15T00:00:00Z'). To get events for a specific day, use getEventsForDay instead which handles timezone math automatically.
  startDate?: string,
  // Calendar ID from listCalendars. If omitted, uses the user's primary calendar. Use 'primary' explicitly for primary calendar.
  calendarId?: string,
...
export async function getUpcomingEvents(
  ...
  return await sdk.callTool("calendar/1.2.0/getUpcomingEvents", params) as getUpcomingEventsOutput;
}
...
export type searchEventsParams = {
  ...
  // Pagination token from previous response's nextPageToken field. Only provide this if you need MORE results beyond what maxResults returned. If maxResults gave you enough events, do NOT fetch additional pages.
  pageToken?: (string | null),
  // RFC3339 formatted timestamp. Returns events that end AFTER this time (i.e., excludes events already finished before this point). MUST include timezone offset matching the user's local timezone, not UTC (e.g., '2024-01-15T00:00:00-05:00' for midnight Eastern Time, NOT '2024-01-15T00:00:00Z').
  startDate?: string,
  // Calendar ID from listCalendars. If omitted, uses the user's primary calendar. Use 'primary' explicitly for primary calendar.
  calendarId?: string,
  // Maximum number of events to return (default 10, reasonable range 1-250)
...
export async function searchEvents(
  ...
  return await sdk.callTool("calendar/1.2.0/searchEvents", params) as searchEventsOutput;
}
...
export async function createEvent(
  ...
  return await sdk.callTool("calendar/1.2.0/createEvent", params) as createEventOutput;
}
...
export async function updateEvent(
  ...
  return await sdk.callTool("calendar/1.2.0/updateEvent", params) as updateEventOutput;
}

/**
 * The type of the input parameter for deleteEvent tool.
 */
export type deleteEventParams = {
  // Email address of the calendar account from listAccounts
  account: string,
  // Event ID from getUpcomingEvents, getEventsForDay, or searchEvents
  eventId: string,
  // Calendar ID from listCalendars. If omitted, uses the user's primary calendar.
  calendarId?: string,
  // Whether to send notifications about the deletion. 'all' sends to all attendees, 'externalOnly' sends only to non-Google Calendar attendees, 'none' sends no notifications. Defaults to 'all'.
  sendUpdates?: ('all' | 'externalOnly' | 'none')
}

/**
 * The type of the output of the deleteEvent tool.
 */
...
/**
 * Delete a calendar event. This permanently removes the event from the calendar. Use with caution as this action cannot be undone.
 * @param sdk - The SDK object.
 * @param params - The parameters for the tool.
 * @returns The result of the tool, matching the type defined by the outputSchema.
 */
export async function deleteEvent(
  ...
  return await sdk.callTool("calendar/1.2.0/deleteEvent", params) as deleteEventOutput;
}
