...
### Pitfall #6: dayjs Only Parses Standard Formats

dayjs natively parses ISO 8601 (`2024-03-15T14:30:00Z`, `2024-03-15`), RFC 2822, and simple date strings like `2024-03-15 14:30`. It does **NOT** parse natural language dates like `"March 15, 2026 at 2:30 PM"` or `"next Tuesday"`. Worse, it fails silently — returning an invalid date object instead of throwing.

This is especially dangerous when designing TypeBox schemas that an LLM fills in (e.g., `completionCallback` params from a sidekick task). LLMs naturally produce human-readable dates, so you must explicitly request ISO 8601 in the field description:

```typescript
// ❌ BAD: LLM will produce natural language dates that dayjs can't parse
startDate: Type.String({
  description: "Start date/time, e.g. 'March 15, 2026 at 2:30 PM'"
})

// ✅ GOOD: LLM will produce parseable ISO format
startDate: Type.String({
  description: "Start date/time in ISO 8601 format, e.g. '2026-03-15T14:30:00'"
})
```
...
// input strings must be ISO 8601, RFC 2822, or "YYYY-MM-DD HH:mm" — NOT natural language
