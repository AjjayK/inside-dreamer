...
For agents with scheduled triggers—especially those running infrequently (daily, weekly)—users shouldn't have to wait until the first scheduled run to experience value. Provide a way for users to trigger the scheduled logic on-demand.

### Implementation

**1. Extract core logic** into a shared function both the cron handler and manual invocation can call:

```typescript
// Core logic - shared between cron and manual invocation
async function generateRecommendations(sdk: ServerSdk) {
  // ... the actual recommendation work
}

// Cron handler (weekly)
export const weeklyCheck = backgroundFunction({
  params: Type.Object({}),
  execute: async (sdk: ServerSdk) => {
    await generateRecommendations(sdk);
  }
});

// Manual trigger - exposed for UI or post-onboarding
export const runNow = backgroundFunction({
  description: "Run the weekly check immediately",
  params: Type.Object({}),
  exported: true,
  execute: async (sdk: ServerSdk) => {
    await generateRecommendations(sdk);
...
**2. Expose to users** via one or more of these approaches:

| Approach | When to use |
|----------|-------------|
| **UI button** ("Check Now", "Refresh") | Most common. Let users trigger on-demand while staying on normal schedule. |
| **Auto-trigger after onboarding** | When immediate results are essential to the experience (e.g., user just configured preferences and expects to see something). |
| **Both** | Auto-run on first setup to show value, then provide button for subsequent manual runs. |

### Examples

| Agent Type | Approach |
|------------|----------|
| Weekend activity finder | "Find Activities" button + optionally auto-run after setting preferences |
| Daily digest | "Preview Today's Digest" button to see what would be sent |
| Weekly report | "Generate Report Now" button to run outside the schedule |
| Price tracker | "Check Prices" button + auto-fetch when items are first added |

### Why This Matters

- **Verification**: Users can confirm the agent works without waiting
- **Debugging**: Easier to catch configuration issues immediately vs. waiting days
