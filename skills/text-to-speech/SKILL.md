...
2. Simplify room codes (334-2-WOPR [VC] → "room WOPR") but keep location names intact
...
9. PRESERVE all proper nouns and names exactly as written - do not expand or change them
...
const paragraphs = cleanedResult.cleanedText
  ...
  .map((text: string) => ({
    text: text.trim(),
    voice: "british-female-1" as const, // Type-safe voice selection
  }));
