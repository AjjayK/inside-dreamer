...
**Widget Mode** (`type: "widget"`): small, 160px to 360px dashboard widget. Show only essential information, compact text, content starts at top. Design for passive consumption, not active use. Avoid scrolling unless requested by the user.
...
- **`views`**: Array of all view types your agent supports. Each view must be registered here to be available
  ...
  - **`constraints`**: Optional size constraints for widget views
...
Widgets are compact, dashboard-embedded views (around 160px to 360px square) designed for passive consumption. They should be visually rich and immediately informative, not complex-apps. Avoid scrolling unless directly requested by the user.

The default widget size is small: around 160px square, but the user can resize it. Ensure they are able to work effectively at that size. If more space is required for this widget, set a minimum and default size that's appropriate for that widget in it's constraints property.
...
## Supporting full-screen on mobile
- Mobile agents are inset by default until they can support full screen rendering. The best agents support full screen. To do this, be sure to follow guidance about safe areas above and add the `viewportFit: cover` property with the `view > type:app` object in the agent.yaml 
...
- **The longer the wait, the more information the user needs.** Use your data plan observations and account for network latency to gauge how long operations take, then design loading states accordingly — a simple indicator may suffice for fast operations, but longer waits call for more context about what's happening and why
