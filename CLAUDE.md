...
By default, all tools your agent uses are **required**—users must approve them during installation. For tools that provide enhanced functionality but aren't essential, you can mark them as **optional** in `agent.yaml`:

```yaml
# agent.yaml
optionalTools:
  - gmail        # Users can disable email features
  - slack        # Users can disable Slack integration
```

**How it works:**
- Tools listed in `optionalTools` show a toggle switch during installation
- Users can enable/disable optional tools at install time and afterward
- If an optional tool is disabled, any calls to it from your agent will be blocked
- Your code should handle gracefully when optional tools are unavailable

**When to make a tool optional:**
- The tool provides "nice to have" features that aren't core to the agent
- Users might not have or want the integration (e.g., not everyone uses Slack)
- The agent can function meaningfully without it

**When to keep a tool required:**
- The agent cannot function without it
- Core features depend on the tool
- There's no reasonable fallback behavior

**Handling disabled optional tools in code:**

```ts
import { slack_post_message } from "@tools/slack";

export const notifyUser = serverFunction({
  execute: async (sdk: ServerSdk, { message }) => {
    // Always save to database (core functionality)
    await db.insert(notifications).values({ message });

    // Try Slack notification (optional enhancement)
    try {
      await slack_post_message(sdk, { channel: "#alerts", text: message });
    } catch (error) {
      // Tool might be disabled - log and continue
      console.log("Slack notification skipped (tool may be disabled):", error.message);
    }
  }
});
```
...
Server functions can take anywhere from under a second to multiple minutes. Design loading feedback proportional to the expected delay — the longer the wait, the more context the user needs about what's happening.
...
| Skill | Description |
|-------|-------------|
| `skills/alignment/SKILL.md` | Understanding and resolving alignment errors. When Sidekick calls or tasks are blocked, review and update your agent's description to match its actual functionality. |
...
| `skills/deep-links/SKILL.md` | Navigation patterns for agents — in-app routing with the History API or React Router, opening the full app from a widget with showInView, and creating external share links (dreamer.com/s/) with content-specific social metadata. |
